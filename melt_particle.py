...
def plot_comparison(dda_data,mie_data,quantity,folder,ax=None):
    if ax is None:
        ...
        for f in dda_data.keys():
            ...
            ax.scatter(1.0e6*DFdda['dipole_spacing'],DFdda[quantity].values,marker='.',label=f)
...
def plot_relative_difference(dda_data,mie_data,quantity,folder,ax=None):
    if ax is None:
        ...
        for f in dda_data.keys():
            ...
            DFdda = dda_data[f]
            DFmie = mie_data[f]
            ax.plot(1.0e6*DFdda['dipole_spacing'],(DFdda.iloc[-1][quantity]-DFdda[quantity])/DFdda.iloc[-1][quantity],label=f)
        ax.set_xlabel('dipole spacing [um]')
